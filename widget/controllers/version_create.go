  ...
  `
	...
	initial_script := `
				function PageViewModel() {
					var self = this;
					self.mainActiveView = ko.observable('##DefaultTemplateId##');

					// modify the PageViewModel
					self.init = function(){
						Sammy(function() {
							/*
								this.get('#:page', function() {
									...
									switch(this.params.page){
										case '##TemplateId##':
												// template specific load conditions here
												break;
										default:
											// default template specific load conditions here
											break;
									}
								});
								*/
	...
	assignment += "        var vm = new PageViewModel();\n        ko.applyBindings(vm);\n"
